...
// There's no out-of-band channel for application data in SSL or TLS.
string read_oob(int|void nbytes, int|void not_all)
{
  return read(nbytes, not_all);
}

static void low_write(string data, int|void priority)
{
  int call_write = !sizeof (write_buffer);
  priority = priority || PRI_application;
  while(sizeof(data))
  {
    Packet packet = Packet();
    packet->content_type = PACKET_application_data;
    packet->fragment = data[..PACKET_MAX_SIZE-1];
    send_packet(packet, priority);
    data = data[PACKET_MAX_SIZE..];
  }
...
int write(string|array(string) s, mixed ... extras)
{
#ifdef SSL3_DEBUG
...
// Simulate out-of-band data by putting it on the urgent queue.
int write_oob(string data, mixed ... extras)
{
#ifdef SSL3_DEBUG
  werror("SSL.sslfile->write_oob\n");
#endif

  if (is_closed || !socket) return -1;

  if (sizeof(extras)) {
    data = sprintf(data, @extras);
  }

  int len = sizeof(data);

  low_write(data, PRI_urgent);
...
private void ssl_read_callback(mixed id, string s)
{
  ...
  if (stringp(data))
  {
    ...
    function(mixed,string:void) rcb = read_callback || read_oob_callback;
    if (!blocking && rcb && sizeof(read_buffer))
      {
...
private void ssl_write_callback(mixed id)
{
  ...
  if ( !res && connected && !blocking)
  {
#ifdef SSL3_DEBUG
    werror("SSL.sslport->ssl_write_callback: Calling write_callback\n");
#endif
    if (write_oob_callback) {
      write_oob_callback(id);
      if (!this_object()) {
	// We've been destructed.
	return;
      }
...
// Note: There's no out-of-band channel in SSL/TLS.
//       We thus need to multiplex the oob callbacks
//       on the ordinary callbacks.

void set_read_oob_callback(function(mixed,string:void) r)
{
#ifdef SSL3_DEBUG
  werror("SSL.sslport: set_read_oob_callback\n");
#endif
  read_oob_callback = r;
  if (sizeof(read_buffer)&& socket)
    ssl_read_callback(socket->query_id(), "");
...
function query_read_oob_callback() { return read_oob_callback; }

void set_write_oob_callback(function(mixed:void) w)
{
#ifdef SSL3_DEBUG
  werror("SSL.sslfile->set_write_oob_callback\n");
#endif
  write_oob_callback = w;
  if (w && socket)
    socket->set_write_oob_callback(ssl_write_callback);
}

function query_write_oob_callback() { return write_oob_callback; }
