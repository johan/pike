...
//"XK_asciicircum":         0x05e,
	...
	return _LookupCharacter(str);
     else if(xkm[str])
        return _LookupCharacter(xkm[str]);
     else return _LookupCharacter("XK_" + str);
  ...
  void makeKeysymtoKeycode(object display)
  {
      keysymtokeycode=([]);
      int i=0;
      for(int i=0; i<sizeof(display->key_mapping); i++)
      {
        if(intp(display->key_mapping[i])) { continue; }
        int mod=0;
  ...
  int LookupKeycode( int keysym, object display )
  {
     ...
     if(sizeof(keysymtokeycode)==0) makeKeysymtoKeycode(display);
     if(keysymtokeycode[keysym]) {
     keycode=keysymtokeycode[keysym][0];
     }
     else keycode=-1;
