		       ...
		       ptrdiff_t pos, int flags)
{
  ...
  default:
	...
	while (data[pos] != ')') {
	  ...
	  case ',':
#if (SHIFT == 0)
	    push_string(make_shared_binary_string(data+start, end-start));
#else /* SHIFT != 0 */
#if (SHIFT == 1)
	    push_string(make_shared_binary_string1(data+start, end-start));
#else /* SHIFT != 1 */
	    push_string(make_shared_binary_string2(data+start, end-start));
#endif /* SHIFT == 1 */
#endif /* SHIFT == 0 */
	    arg++;
	    start = pos;
